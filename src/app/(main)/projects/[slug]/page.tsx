        ...
        <>
            ...
            <div className="col-span-12 lg:col-span-3 lg:col-start-9">
                <div className="lg:sticky lg:top-0 bg-neutral-50 border border-neutral-200 p-6 rounded-lg">
                    ...
                    {p.metadata.date && (
                        <div className="mb-4">
                            <div className="text-xs font-semibold text-neutral-600 mb-1">Date</div>
                            <div className="text-sm text-neutral-700">{new Date(p.metadata.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}</div>
                        </div>
                    )}

                    {p.metadata.github && (
                        <div className="mb-4">
                            <div className="text-xs font-semibold text-neutral-600 mb-1">Repository</div>
                    ...
                    {p.metadata.tags && p.metadata.tags.length > 0 && (
                        <div className="mb-4">
                            <div className="text-xs font-semibold text-neutral-600 mb-1">Tags</div>
                            <div className="flex flex-wrap gap-1">
                                {p.metadata.tags.map((tag: string) => (
                                    <span key={tag} className="text-xs bg-primary-50 border border-primary-200 text-primary-700 px-2 py-1 rounded-full">
                                        {tag}
                                    </span>
                                ))}
                            </div>
